...
public class customMetrics implements Runnable {
    ...
    private void sendPost()
    {
        ...
        Gdx.net.sendHttpRequest(httpRequest, new Net.HttpResponseListener()
        {
            public void handleHttpResponse(Net.HttpResponse httpResponse) {
                BetterNote.logger.info("Metrics: http request response: " + httpResponse.getResultAsString());
            }

            public void failed(Throwable t) {
                BetterNote.logger.info("Metrics: http request failed: " + t.toString());
            }

            public void cancelled() {
                BetterNote.logger.info("Metrics: http request cancelled.");
            }
        });
    }
    ...
    public void run()
    {
        for(HashMap map : CardCrawlGame.metricData.event_choices){
            if(map.get("event_name").equals(BetterNoteEvent.ID)){
                foundEvent = true;
